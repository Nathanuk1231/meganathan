...
export function renderProfile() {
  ...
  if (!currentUser) {
    div.innerHTML = `<div class="card" style="text-align:center;padding:50px;">
      <p style="font-size:2rem;margin-bottom:12px;">👤</p><h2>Not signed in</h2>
      <p class="muted" style="margin:10px 0 18px;">Sign in to view your profile</p>
      <button class="btn btn-p" onclick="auth.doLogin()">Sign In with Google</button></div>`;
    return;
  }
  ...
  db.ref('profiles/' + currentUser.uid).once('value').then(s => {
    ...
    if (p.uid !== currentUser.uid) {
      div.innerHTML = `<div class="card" style="padding:30px;text-align:center;">
        <p style="color:var(--danger);">⚠️ Profile mismatch error</p></div>`;
      return;
    }
    ...
    div.innerHTML = `<div class="card" style="padding:30px;text-align:center;">
      <p style="color:var(--danger);">⚠️ Could not load profile</p>
      <p class="muted" style="font-size:0.82rem;margin-top:8px;">${esc(e.message)}</p>
    </div>`;
  });
}
...
export function setStatus(s) {
  ...
  db.ref('profiles/' + currentUser.uid).update({ status: s });
  ...
  div.innerHTML = `
    <div class="modal">
      ...
      <div style="display:flex;align-items:center;gap:14px;margin-bottom:14px;">
        ...
        <div>
          <p class="muted" style="font-size:0.78rem;margin-bottom:6px;">Upload photo or paste URL</p>
          <label class="btn btn-sm" style="cursor:pointer;">📁 Upload<input type="file" id="editAvFile" accept="image/*" style="display:none;" onchange="profile.prevEditAv(this)"/></label>
        </div>
      </div>
      <div class="form-row">
...
export function prevEditAv(inp) {
  ...
  if (f.size > 1.2 * 1024 * 1024) {
    toast('Photo must be under 1.2MB', 'err');
    inp.value = '';
    return;
  }
...
export function saveProfile() {
  ...
  const name = sanitizeInput(document.getElementById('editName').value, 30);
  ...
  db.ref('profiles/' + uid).once('value').then(s => {
    ...
    document.getElementById('pubProfileContent').innerHTML = `
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        ${currentUser && uid !== currentUser.uid ? `<button class="btn btn-p btn-sm" onclick="friends.addFriend('${uid}','${esc(name)}')">➕ Add Friend</button>` : ''}
      </div>
    `;
...
export function checkStreak(uid) {
  ...
  db.ref('profiles/' + uid).once('value').then(s => {
    ...
    if (updated) {
      db.ref('profiles/' + uid).update({ streak, lastLogin: ts() });
      if (streak >= 2) {
