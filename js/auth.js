...
function checkAdminStatus(uid) {
  if (!validateUID(uid)) { isUserAdmin = false; return; }
  db.ref('admins/' + uid).once('value').then(s => {
    isUserAdmin = s.val() === true;
    if (window.auth) updateNav(currentUser, currentProfile);
  }).catch(() => { isUserAdmin = false; });
}
...
export function doLogout() {
  if (currentUser) db.ref('online/' + currentUser.uid).remove();
  auth.signOut().then(() => {
    currentUser = null;
    currentProfile = null;
    updateNav(null);
...
auth.onAuthStateChanged(user => {
  ...
  if (user) {
    ...
    onRef.set({ name: user.displayName || 'User', uid: user.uid, status: 'online', at: ts() });
...
export function updateCurrentProfile(updates) {
  if (currentProfile) currentProfile = { ...currentProfile, ...updates };
}
