var canvas, context, game, snake, food;
...
game = {
  ...
  stop: function() {
    game.over = true;
    game.message = 'GAME OVER · SPACE TO RESTART';
  },
  ...
  drawScore: function() {
    ...
    context.font = (canvas.height) + 'px monospace';
  ...
  drawMessage: function() {
    if (game.message !== null) {
      ...
      context.font = (canvas.height / 12) + 'px monospace';
      context.textAlign = 'center';
...
snake = {
  ...
  checkCollision: function() {
    if (snake.isCollision(snake.x, snake.y) === true) { game.stop(); }
  },
...
var inverseDirection = { 'up': 'down', 'left': 'right', 'right': 'left', 'down': 'up' };
var keys = {
  up: [38, 75, 87], down: [40, 74, 83], left: [37, 65, 72], right: [39, 68, 76], start_game: [13, 32]
};

function getKey(value) {
  for (var key in keys) {
    if (keys[key] instanceof Array && keys[key].indexOf(value) >= 0) { return key; }
  }
  return null;
}
